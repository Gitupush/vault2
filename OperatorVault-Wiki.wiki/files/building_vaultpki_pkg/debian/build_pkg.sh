...
copy_certifcates()
{
    echo "Target env set to: ${PKG_TARGET}"

    # Set copy targets  depending on target env
    if  [[ "${PKG_TARGET}" == "prod" ]]; then
        target_arr=("dal" "lon" "tok")
    elif [[ "${PKG_TARGET}" == "dev" ]]; then
        target_arr=("pok" "stg")
    else
        error_abort "FATAL: No target env passed. Aborting"
    fi

    rm -rf vaultpkicerts/usr/local/share/ca-certificates/*.${CRT_EXTNS}
    mkdir -p vaultpkicerts/usr/local/share/ca-certificates
  
    for geo in "${target_arr[@]}"
    do
        cp ${CRT_PATH}/vault-$geo-root-ca.crt vaultpkicerts/usr/local/share/ca-certificates/
        cp ${CRT_PATH}/vault-$geo-intermediary-ca.crt vaultpkicerts/usr/local/share/ca-certificates/
    done
...
move_pkg(){
    ...
    mv vaultpkicerts.deb ${PKG_OP_DIR}/build/vaultpkicerts_${PKG_TARGET}_${PKG_VER}_amd64.deb
}
